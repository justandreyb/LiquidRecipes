...
public class FlavorController {
    ...
    List<FlavorDto> getTopNumberFlavors(@RequestParam int number) {
        return flavorMapper.toFlavorDtos(flavorService.getTop(number));
    }

    @GetMapping("/{id}")
    FlavorDto getFlavor(@RequestParam String id) {
        return flavorMapper.toFlavorDto(flavorService.get(id));
    }

    @GetMapping("/{id}/comments")
    List<CommentDto> getFlavorComments(@RequestParam String id) {
        return commentMapper.toCommentDtos(commentService.getAllByFlavor(id));
    }

    @GetMapping("/{id}/likes")
    List<LikeDto> getFlavorLikes(@RequestParam String id) {
        return likeMapper.toLikeDtos(likeService.getAllByFlavor(id));
    }

    @GetMapping("/{id}/image")
    ImageDto getFlavorImage(@RequestParam String id) {
    ...
    void addCommentToFlavor(@RequestParam String id, @RequestBody CommentDto comment) {
    ...
    void addLikeToFlavor(@RequestParam String id, @RequestBody LikeDto like) {
        likeService.addToFlavor(id, likeMapper.fromLikeDto(like));
    }

    @PostMapping("/{id}/image")
    void addLikeToFlavor(@RequestParam String id, @RequestBody ImageDto image) {
        imageService.addToFlavor(id, imageMapper.fromImageDto(image));
    }

    @DeleteMapping("/{id}")
    void deleteFlavor(@RequestParam String id) {
        flavorService.delete(id);
    }

    @DeleteMapping("/{id}/comments/{commentId}")
    void deleteCommentFromFlavor(@RequestParam String commentId) {
        commentService.delete(commentId);
    }

    @DeleteMapping("/{id}/likes/{likeId}")
    void deleteLikeFromFlavor(@RequestParam String likeId) {
