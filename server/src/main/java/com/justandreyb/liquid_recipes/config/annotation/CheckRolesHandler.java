...
@Aspect
@Component
public class CheckRolesHandler {

    private boolean isAllowed(Set<String> roles) {
        Collection<? extends GrantedAuthority> currentRoles =
            SecurityContextHolder.getContext().getAuthentication().getAuthorities();
        return currentRoles.stream().anyMatch(authority -> roles.contains(authority.getAuthority().toLowerCase()));
    }
    ...
    private static class CheckRolesAnnotation {
        ...
        CheckRolesAnnotation(Method method) {
            ...
            this.roles = Arrays.stream(checkRoles.value()).map(Roles::getValue).collect(Collectors.toSet());
        }
    }
}
